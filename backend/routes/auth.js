...
// Import all controller functions for authentication
const { 
    signup, 
    login, 
    updateProfile, 
...
// --- Standard Email, Password, and Profile Routes ---
router.post('/signup', signup);
router.post('/login', login);
router.put('/profile', auth, updateProfile);
...
// --- Route to get current user data from a token ---
router.get('/me', auth, async (req, res) => {
    try {
        // req.user.id is attached by the authMiddleware
        const user = await User.findById(req.user.id).select('-password');
...
router.get('/google/callback', passport.authenticate('google', { 
    failureRedirect: 'https://reunitelnds.netlify.app/', // Redirect home on failure
    session: false // Use JWT, not sessions
}), (req, res) => {
    const payload = { user: { id: req.user.id } };
    jwt.sign(
        payload,
        'THIS_IS_A_VERY_LONG_AND_SECRET_KEY_FOR_MY_APP_12345', // Should be an environment variable
        { expiresIn: '1h' },
        (err, token) => {
            if (err) throw err;
